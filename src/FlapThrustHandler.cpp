...
namespace IDRC {
    void FlapThrustHandler::Register() {
        if (m_isRegistered) {
            log::warn("IDRC - {}: FlapThrustHandler already registered", __func__);
            return;
        }
        ...
        if (bSuccess) {
            m_isRegistered = true;
            log::info("IDRC - {}: Registered FlapThrustHandler", __func__);
        } else {
            ...
            if (!bSinked) {
                log::info("IDRC - {}: Failed to register FlapThrustHandler", __func__);
            }		
        }
    }
    ...
    void FlapThrustHandler::Unregister() {
        if (m_isRegistered) {
            ...
            log::info("IDRC - {}: Unregistered FlapThrustHandler", __func__);
        } else {
            log::warn("IDRC - {}: FlapThrustHandler was not registered", __func__);
        }
    }
